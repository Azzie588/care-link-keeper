...
export const createProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (locations && locations.length) {
      ...
      if (rows.length) {
        const { error: locErr } = await context.supabase
...
export const updateProvider = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    providerInput.extend({ id: z.string().uuid() }).parse(d),
  )
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error(error.message);
    if (locations) {
      await context.supabase.from("provider_locations").delete().eq("provider_id", id);
      const rows = locations
        .filter((l) => l.address_line1 || l.city || l.label)
...
export const searchNpi = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => npiQuery.parse(d))
  .handler(async ({ data }): Promise<NpiResult[]> => {
